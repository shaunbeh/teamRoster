...
express.get("/api/data", async (req, res) => {
  try {
    ...
    try {
      ...
      res.json({
        status: sheetsError,
        message: "Error While Retrieving Google Sheet, Retrieved Latest Cached Roster Instead",
        data: cachedRoster,
      });
    } catch (nodeError) {
      res.json({
        ...
        message: "Error While Retrieving Google Sheet and Error Retrieving Cached Roster",
        data: null,
      });
    }
  }
